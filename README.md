...
### 具体 Case 1：成功定位普通过程错误

**公开样本：** ProcessBench `gsm8k-0`，由 `Qwen2-7B-Instruct` 生成候选解答。

**题目：** 周五草坪上有 18 只粉色火烈鸟。周六取走其中三分之一，涂成白色后放回；周日又加入 18 只粉色火烈鸟。周日中午，粉色比白色多多少只？

**模型候选解答：**

1. 周五有 18 只粉色火烈鸟。
2. 周六取走 `18 × 1/3 = 6` 只并涂成白色，因此剩下 12 只粉色、增加 6 只白色；但该步骤随后错误地写成“有 `12 + 6 = 18` 只粉色和 6 只白色”。
3. 周日再加入 18 只粉色，据此错误地得到 36 只粉色、6 只白色。
4. 最终计算 `36 - 6 = 30`。

**ProcessBench Oracle：** 第一处错误是步骤 1（从 0 开始编号），因为 `12 + 6 = 18` 是总数，不是粉色数量。正确状态应为 12 只粉色和 6 只白色。

**Gemini Flash：** 预测步骤 1，confidence `1.0`。模型明确指出该步骤把 18 只总数误写成 18 只粉色，因此这一 case **定位正确**。

### 具体 Case 2：最终答案正确掩盖了过程错误

**公开样本：** ProcessBench `gsm8k-290`，由 `Qwen2-7B-Instruct` 生成候选解答。

**题目：** Zack 的柜子是 Timothy 的一半；Peter 的柜子是 Zack 的四分之一。Peter 的柜子为 5 立方英寸，Timothy 的柜子有多大？

**模型候选解答：**

1. 先写出 `5 ÷ (1/4) = 20`，计算上正确得到 Zack 的柜子为 20；但同一句解释却称 Zack 的柜子是 Peter 的“**两倍**”，与题意和乘四计算自相矛盾。
2. Zack 是 Timothy 的一半，因此 `20 × 2 = 40`，最终答案 40 正确。

**ProcessBench Oracle：** 第一处错误是步骤 0。错误不是最终数值，而是步骤内部把“四倍”说成“两倍”；这是典型的“结论正确但局部推理陈述无效”。

**Gemini Flash：** 预测 `-1`，confidence `1.0`，并声称“两步在数学和逻辑上都正确”。它只跟随了正确计算和最终答案，没有检查同一步自然语言与算式是否一致，因此这一 lucky-answer case **漏检**。

这两个 case 使用相同输入 contract 和同一个 Verifier：前者说明模型具备常规错误定位能力，后者则隔离出更具体的 Reward-Hacking 风险，而不是用总体平均分掩盖失败模式。
