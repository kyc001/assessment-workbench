...
- SQLite 工作区、运行记录、成对阶段事件、Artifact 原子发布、checkpoint 恢复和人工决策。
- Fixture、MinerU CLI、MinerU HTTP 同步解析适配器。
- 标题层级知识图谱、可选结构化语义知识抽取和来源证据。
- OpenAI-compatible JSON Schema 模型调用、SSE 接收、ContextPack 和完整调用审计。
- 关键词检索、图谱一层扩展和知识点标签浏览。
- 基于知识点标签的 `QuestionSpec` 规划。
- Prompt、Subject Capability、Subject Research、Reviewer、Tool 和 Validator 注册表。
- 高考数学锁定 19 题结构；能力包不包含静态题目、答案或 Rubric。
- 未知科目的三角色动态研究池、quorum 综合、字段证据追踪和可恢复实时 manifest。
- Blueprint 结构化保存计算器政策与难度统计口径，并支持版本化人工覆盖。
...
- GUI 已覆盖核心运行与编辑操作；Blueprint 编辑、页面级人工验收和版本差异仍待补齐。
...
- [x] 支持列出、读取和校验产物完整性。
- [x] `BEGIN IMMEDIATE` 串行化并发版本分配，元数据失败补偿删除本次文件。
- [x] 受限 reconcile 清理临时文件和可识别的孤立版本文件。
...
- [x] completed PhaseEvent 与 checkpoint 使用同一 SQLite 事务提交。
- [ ] 为非幂等阶段定义补偿或禁止自动恢复。
...
- [ ] 提供 `physics.yaml`。
- [x] 提供首个高考数学能力包，锁定 19 题、150 分结构且不包含静态题目、答案或 Rubric。
- [x] 未注册科目由 Subject Research Agent 动态生成 Profile 和 Blueprint。
- [x] 课程范围、施测设计和质量政策使用独立 child 并行研究，单个失败不取消成功结果。
- [x] 成功研究报告即时写 Artifact 和 live manifest，恢复只重跑失败或缺失角色。
- [x] quorum 后生成带 claim/evidence/field trace 的结构化研究综合，并等待人工确认。
- [x] 加载时校验 Reviewer、工具、Validator 和 Prompt 是否已注册。
- [x] 建立版本化 Prompt Registry 与 Subject Capability Registry。
- [ ] 定义 `SubjectTool` 协议。
- [ ] 接入 SymPy 符号验证。
- [ ] 实现物理量纲/单位检查器。
- [ ] 实现数值随机采样和边界情况检查器。
...
- [x] `ReviewReport/ReviewFinding`。
- [x] `ArbitrationDecision` 与分角色修订反馈。
- [x] 每个版本保存 parent version、生成角色、模型、Prompt 版本和来源引用。

## M5.2 状态机

- [x] `QUESTION_INITIALIZING`。
- [ ] `CONTEXT_RETRIEVING`。
- [x] `QUESTION_PLANNING` 的前置 `QuestionSpec` 工作流。
- [x] `PROBLEM_GENERATING`。
- [x] `SOLUTION_GENERATING`，使用独立 Agent。
- [x] `RUBRIC_GENERATING`。
- [x] `MULTI_REVIEWING`。
- [x] `ARBITRATING`。
- [x] `QUESTION_FINALIZING`。
...
- [x] Subject Reviewer。
- [x] Solvability Reviewer。
- [x] Rubric Reviewer。
- [x] Pedagogical Reviewer。
- [x] Structure Reviewer，确定性实现。
- [ ] Similarity Reviewer，比较往年题和同卷题。
- [x] 独立 Reviewer 并行执行并分别记录模型调用。
- [x] 每个 Reviewer 使用独立 grandchild run、即时 manifest 和输入版本签名。

## M5.5 仲裁和重试

- [x] 决策：`PASS`。
- [x] 决策：`PASS_WITH_WARNINGS`。
- [x] 决策：`RETRY_PROBLEM`。
- [x] 决策：`RETRY_SOLUTION`。
- [x] 决策：`RETRY_RUBRIC`。
...
- [x] 决策：`ESCALATE_HUMAN`。
- [x] 决策：`ABORT`。
- [x] Pydantic/JSON Schema 验证裁决与错误严重度组合。
- [x] 分类路由正确失效下游版本。
- [x] 题干、解答、Rubric 和总轮次使用独立预算。
- [x] 超过预算保留最新 Bundle 和证据并进入人工处理，不无限循环。
...
- [x] 校验总分、各区分值、覆盖分值和题量。
- [x] 计算器政策 `calculator_policy` 和难度统计口径 `difficulty_basis`。
- [x] 标题、施测政策和 QuestionPlan 支持版本化人工修改与 checkpoint CAS 覆盖。
- [ ] 支持目标年级、考试时长、语言和材料范围。
- [x] Agent 动态规划来源；用户 YAML 约束接入仍待完善，示例 YAML 不包含预设题目。
...
- [x] 每道题使用独立子 run 和 parent run。
- [x] 有界并发，不超过题目并发和共享模型请求配置。
- [x] 单题失败不重做整卷，成功题立即写 editable Bundle。
- [x] 每道题即时更新父级 manifest；指定题号可单独重跑和重新接纳编辑版本。
- [x] 单题内部 Writer/Solver/Rubric 使用独立可恢复阶段。
- [x] Reviewer grandchild 并行执行，Arbiter 按依赖图回跳。
- [x] `EXAM_ASSEMBLING`，只组装 Agent 生成并审核通过的版本化题目。
...
- [x] 来源与版权风险审核。
- [x] 排版结构审核。
- [x] 仲裁返回具体 `question_ids/section_ids`。
- [x] 支持 `REPLACE_QUESTIONS/REBALANCE_DIFFICULTY/REBALANCE_COVERAGE/REGENERATE_SECTION/ESCALATE_HUMAN`。
- [x] `WAITING_EXAM_APPROVAL`。
- [x] 全部 Reviewer 通过且无 `error/fatal` 时，warning 只能形成 `PASS_WITH_WARNINGS`，不得触发自动返工。
...
- [x] LaTeX 普通文本转义、数学命令安全检查和领域结构确定性检查。
...
- [x] `questions plan`。
- [x] `questions generate` 能力由整卷逐题子流程提供；独立 CLI 仍待增加。
- [ ] `exams plan`。
- [x] `exams generate`。
- [ ] `exams export`。
...
- [ ] 阶段耗时和模型成本汇总。
- [x] Prompt、strict Schema、结构化 user payload 和输入 Artifact 的 ContextPack 快照。
- [ ] 可选 OpenTelemetry/Langfuse 适配器，不成为核心依赖。

## 安全和隐私

- [x] API Key 不进入 artifact、日志或模型调用 payload。
...
- [x] 本地单用户 FastAPI 服务层和 React GUI。
- [ ] OpenAPI 生成客户端，不手工复制 DTO。
- [ ] 持久任务队列和多 worker 安全执行。
- [x] SSE 阶段快照、轮询降级和断线重连。
- [x] 明确本地工作台不引入登录、管理员、角色、Token、权限或多租户。
- [ ] 材料、知识图谱、题目、试卷和答卷 API。
- [x] 工作流时间线、轮次、研究、逐题状态和人工检查点 UI。
- [x] PDF、Rubric、Artifact 和审计 JSON 查看器。
- [ ] 阅卷三栏人工复核界面。
...
- [x] A1：动态 Subject Research Agent 生成 Subject Profile 候选和来源说明。
- [x] A2：Exam Blueprint Agent 根据用户约束和材料生成结构化蓝图。
- [x] A3：Question Writer、Independent Solver 和 Rubric Builder 逐题生成版本化产物。
- [x] A4：Reviewer Pool、Arbiter 和分类重试形成有界闭环。
- [x] A5：从审核通过的领域对象生成 LaTeX，并由 Tectonic 编译 PDF。
- [x] A6：拆分版本化 Prompt Bundle，建立 Subject Capability、Reviewer、Tool 和 Validator Registry，并接入高考数学锁定结构。
- [x] A7：拆分单题内部可恢复阶段，Reviewer 使用独立 grandchild run 并按输入版本复用。
- [x] A8：增加整卷确定性与语义审核、结构化仲裁、目标计划修订和局部 child 替换。
- [x] A9：增加模型 ContextPack、扩展调用审计、Artifact 并发发布事务和阶段原子提交。
- [x] A10：完成未知科目多角色动态研究池、结构化施测政策、模型分层和 warning 仲裁护栏。
- [x] A11：以未注册的高等数学主题真实生成 18 题、100 分整卷，并逐页验收题目、答案和 Rubric PDF。
